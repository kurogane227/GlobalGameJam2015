...
public class Thruster : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (desiredThrust != previousThrust) 
		{
			...
			if (desiredThrust >= previousThrust)
			{
				actualThrust = previousThrust + ((desiredThrust - previousThrust) * Time.deltaTime * 12f);
			}
			else
			{
				actualThrust = previousThrust + ((desiredThrust - previousThrust) * Time.deltaTime * 15f);
			}
	...
	{
		float desiredParticleScale = (desiredThrust / 2f);
		...
		if (particleTransform.localScale.x < desiredParticleScale)
		{
			...
			particleTransform.GetComponentInChildren<Light>().range = newScale * 25f;
		}
